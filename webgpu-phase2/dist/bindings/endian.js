...
export function beFieldToBigInt(bytes, offset = 0) {
    let result = 0n;
...
export function bigIntToBeField(value, out, offset = 0) {
 ...
 */
export function bigIntToU32Array(value) {
 ...
 */
export function u32ArrayToBigInt(limbs, offset = 0) {
    let result = 0n;
 ...
 */
export function beBytesToU32(bytes, offset = 0) {
    const view = new DataView(bytes.buffer, bytes.byteOffset + offset, FP_BYTES);
    const limbs = new Uint32Array(LIMBS_PER_FIELD);
    for (let i = 0; i < LIMBS_PER_FIELD; i++) {
 ...
 */
export function u32ToBeBytes(limbs, limbOffset, out, byteOffset) {
    const view = new DataView(out.buffer, out.byteOffset + byteOffset, FP_BYTES);
    for (let i = 0; i < LIMBS_PER_FIELD; i++) {
 ...
 */
export function gnarkG1ToU32(bytes, offset = 0) {
 ...
 */
export function u32ToGnarkG1(limbs, limbOffset = 0) {
 ...
 */
export function gnarkG1BatchToU32(bytes, numPoints) {
 ...
 */
export function u32BatchToGnarkG1(limbs, numPoints) {
    const pointBytes = FP_BYTES * 2;
    const pointLimbs = LIMBS_PER_FIELD * 2;
