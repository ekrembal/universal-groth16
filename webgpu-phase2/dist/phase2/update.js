 ...
 */
import { g1ScalarMul, g2ScalarMul, scaleG1Slice, frInv, } from '../crypto.js';
 ...
 */
export function phase2UpdateCPU(p, delta, sigmas) {
    // 1. Scale sigma-related parameters
    for (let i = 0; i < sigmas.length; i++) {
        p.parameters.g2.sigma[i] = g2ScalarMul(p.parameters.g2.sigma[i], sigmas[i]);
        p.parameters.g1.sigmaCKK[i] = scaleG1Slice(p.parameters.g1.sigmaCKK[i], sigmas[i]);
    }
    // 2. Scale delta
 ...
 */
export async function phase2UpdateGPU(p, delta, sigmas, device) {
    // 1. Scale sigma-related parameters (CPU — few points)
    for (let i = 0; i < sigmas.length; i++) {
        p.parameters.g2.sigma[i] = g2ScalarMul(p.parameters.g2.sigma[i], sigmas[i]);
        // SigmaCKK might be large — use GPU if available
        p.parameters.g1.sigmaCKK[i] = await scaleG1SliceBatched(p.parameters.g1.sigmaCKK[i], sigmas[i], device);
    }
    ...
    const deltaInv = frInv(delta);
    p.parameters.g1.z = await scaleG1SliceBatched(p.parameters.g1.z, deltaInv, device);
    p.parameters.g1.pkk = await scaleG1SliceBatched(p.parameters.g1.pkk, deltaInv, device);
}
/**
 ...
 */
export async function computeChallenge(data) {
