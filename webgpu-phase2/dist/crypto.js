 ...
 */
import { FP_MODULUS, FR_MODULUS, } from './constants.js';
// ===========================================================================
...
export function fp2Square(a) {
    return fp2Mul(a, a);
}
export function fp2Neg(a) {
    return { c0: fpNeg(a.c0), c1: fpNeg(a.c1) };
}
/** (c0 + c1·i)⁻¹ = (c0 − c1·i) / (c0² + c1²) */
export function fp2Inv(a) {
 ...
 */
export function fp2LexLargest(a) {
    if (a.c1 !== 0n) {
...
export function frInv(a) {
    if (a === 0n)
        throw new Error('frInv: division by zero');
    return modPow(a, FR_MODULUS - 2n, FR_MODULUS);
}
/** (a × b) mod r */
export function frMul(a, b) {
    return (a * b) % FR_MODULUS;
}
/** Normalize scalar to [0, r) */
export function frNormalize(a) {
...
export function g1IsInfinity(p) {
...
const JAC_INFINITY = { x: 0n, y: 1n, z: 0n };
 ...
 */
export function g1ScalarMul(point, scalar) {
    scalar = frNormalize(scalar);
    if (scalar === 0n || g1IsInfinity(point))
        return { x: 0n, y: 0n };
    let result = { ...JAC_INFINITY };
 ...
 */
export function scaleG1Slice(points, scalar) {
...
export function g2IsInfinity(p) {
...
const JAC_G2_INFINITY = {
    x: { c0: 0n, c1: 0n },
    y: { c0: 1n, c1: 0n },
    z: { c0: 0n, c1: 0n },
};
function toJacobianG2(p) {
    if (g2IsInfinity(p))
        return { ...JAC_G2_INFINITY };
    return { x: p.x, y: p.y, z: { c0: 1n, c1: 0n } };
}
function toAffineG2(p) {
    if (fp2IsZero(p.z)) {
        return {
            x: { c0: 0n, c1: 0n },
            y: { c0: 0n, c1: 0n },
...
function jacDoubleG2(p) {
    ...
    const D = fp2MulScalar(fp2Sub(fp2Square(xPlusB), fp2Add(A, C)), 2n);
...
function jacAddG2(p, q) {
    ...
    const y3 = fp2Sub(fp2Mul(r, fp2Sub(v, x3)), fp2MulScalar(fp2Mul(s1, j), 2n));
    const zSum = fp2Add(p.z, q.z);
    const z3 = fp2Mul(fp2Sub(fp2Square(zSum), fp2Add(z1z1, z2z2)), h);
    return { x: x3, y: y3, z: z3 };
}
/**
 ...
 */
export function g2ScalarMul(point, scalar) {
    scalar = frNormalize(scalar);
    if (scalar === 0n || g2IsInfinity(point)) {
        return { x: { c0: 0n, c1: 0n }, y: { c0: 0n, c1: 0n } };
    }
    let result = { ...JAC_G2_INFINITY };
