...
/**
 * Cached pipeline state to avoid recompiling shaders for the same device.
 */
interface PipelineCache {
  device: GPUDevice;
  pass1Pipeline: GPUComputePipeline;
  intermediatePipeline: GPUComputePipeline;
 ...
 */
function getOrCreatePipelines(device: GPUDevice): PipelineCache {
 ...
 */
export async function gpuBatchScalarMul(
  device: GPUDevice,
  affinePointsU32: Uint32Array,
  scalarU32: Uint32Array,
  numPoints: number,
): Promise<Uint32Array> {
  ...
  try {
    ...
    // ---- Read back results ----
    const resultU32 = await readBufferU32(
      device,
      affineOutputBuf,
      numPoints * AFFINE_LIMBS,
    );
