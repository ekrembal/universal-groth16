 ...
 */
export function makePass1Entry(workgroupSize: number): string {
  return /* wgsl */ `
@group(0) @binding(0)
 ...
 */
export function makeIntermediateEntry(workgroupSize: number): string {
  return /* wgsl */ `
@group(0) @binding(0)
 ...
 */
export function makeFinalEntry(workgroupSize: number): string {
  return /* wgsl */ `
@group(0) @binding(0)
var<storage, read> inResults: array<Point>;
@group(0) @binding(1)
...
export function getPass1Shader(workgroupSize = WORKGROUP_SIZE): string {
  return BASE_MODULES + makePass1Entry(workgroupSize);
}

/** Full shader code for intermediate passes (next 64 scalar bits). */
export function getIntermediateShader(workgroupSize = WORKGROUP_SIZE): string {
...
export function getFinalShader(workgroupSize = WORKGROUP_SIZE): string {
