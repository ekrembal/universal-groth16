...
import { FP_MODULUS } from '../constants.js';
import { bigIntToBeField } from '../bindings/endian.js';
import { fpLexLargest, fp2LexLargest } from '../crypto.js';
import type { G1Affine, G2Affine, UpdateProof, Phase2 } from './types.js';
...
class BinaryWriter {
  ...
  writeG1Raw(point: G1Affine): void {
  ...
  writeG2Raw(point: G2Affine): void {
  ...
  writeG1Slice(points: G1Affine[]): void {
  ...
  writeG1SliceRaw(points: G1Affine[]): void {
    this.writeUint32(points.length);
    for (const p of points) {
      this.writeG1Raw(p);
    }
  ...
  writeUpdateProof(proof: UpdateProof): void {
  ...
  writeBytesShort(data: Uint8Array): void {
    if (data.length > 255) {
 ...
 * @returns raw bytes
 */
export function serializePhase2(p: Phase2): Uint8Array {
 ...
 */
export function serializePhase2Raw(p: Phase2): Uint8Array {
  const writer = new BinaryWriter();
  const nbCommitments = p.parameters.g2.sigma.length;
